...
def main():
    ...
    sql = '''SELECT DISTINCT
                ...
                f.pair_order 
            FROM
                small_panel_patient_positive_results r
                LEFT JOIN rpt_mutation_desc d ON r.id = d.report_mutation_id
                LEFT JOIN report_mutation_fusion_desc f ON r.id = f.report_mutation_id 
            WHERE
                r.selected = 1 and r.frequency <> '-';'''
