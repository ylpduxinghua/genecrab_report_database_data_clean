...
def main():
    ...
    sql = '''SELECT
                ...
                q.qc_value 
            FROM
                small_panel_patient_positive_results m
                left JOIN rpt_mutation_desc d ON m.id = d.report_mutation_id
                left JOIN rpt_variant_qc q ON d.id = q.report_mutation_desc_id 
            WHERE
                m.selected = 1 
                AND m.data_type = 'SNV' 
                AND m.frequency <> '-' 
    ...
    df.to_csv('small_panel_snv_qc.tsv', sep='\t')
    ...
    # 创建大panel SNV QC表
    conn = get_database_conn()
    small_panel_snv_qc_sql = '''create table small_panel_snv_qc_clean_final (
        id int(10) not null auto_increment,
        report_id int(10) not null comment "reports表id",
        report_sample_id int(10) not null comment "report_samples表id",
        report_mutation_id int(10) not null comment "small_panel_patient_positive_results表id",
    ...
    create_table(conn, 'small_panel_snv_qc_clean_final', small_panel_snv_qc_sql)
    conn.close()

    # 插入数据
    conn = get_database_conn()
    cur = conn.cursor()
    insert_sql = f'''insert into small_panel_snv_qc_clean_final (
